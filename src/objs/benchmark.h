...
#define benchmark(title, runs, ...) \
	std::list<std::reference_wrapper<benchmark::Observer>> BENCHMARK_GENERATE_LABEL(benchmark_observers){__VA_ARGS__}; \
	for (auto& o : BENCHMARK_GENERATE_LABEL(benchmark_observers)) {\
